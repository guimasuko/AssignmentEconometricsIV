...
We will follow the FRED-MD classification of variables into 8 groups: (i) output and income; (ii) labor market; (iii) housing; (iv) consumption,
orders and inventories; (v) money and credit; (vi) interest and exchange rates; (vii) prices; and (viii) stock market. We are adding a ninth group called
(ix) lags , with the lagged inflation series.
...
coeff_lasso <- data.frame(matrix(ncol = ncol(reg_data), nrow = 261))
colnames(coeff_lasso) <- colnames(reg_data)
coeff_lasso <- coeff_lasso %>% select(-inflation.ahead) 
...
for (bb in 1:261){
  a = var_lasso[[bb]] %>% unlist()
  b = co_lasso[[bb]] %>% unlist()
    for (c in 2:length(a)){
      coeff_lasso[bb,a[c]]<-b[c]
    }
}
...
#Multiply for sd
for (bb in 1:261){
  coeff_lasso[,bb] = coeff_lasso[,bb]*sd(reg_data2[,bb])
}
