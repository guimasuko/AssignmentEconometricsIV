...
for(a in 1:(length(inflation)-rolling_window)){
  ...
  # Prediction
  new = reg_data %>% select(-inflation.ahead) %>% tail(1)
  forecast1[a] = predict(ridge, newx = data.matrix(new), s=ic_ridge$lambda)
}
...
```{r forecast_loop_LASSO_test, eval=FALSE}
# Rolling window forecasting
rolling_window <- 492
...
for(a in 1:(length(inflation)-rolling_window)){
  ...
  # embed
  reg_data = my_embed(train, n_lags = 1) 
  cpi_lags = my_embed(train_cpi, n_lags = 24)
  # bind the embeded columns with the one-step-ahead inflation
  ...
  # LASSO estimation
  ic_lasso <- ic.glmnet(x = reg_data %>% select(-inflation.ahead), 
                        y = reg_data$inflation.ahead, 
                        crit="bic", alpha = my_alpha)
  lasso <- glmnet(x = reg_data %>% select(-inflation.ahead), 
                  y = reg_data$inflation.ahead,
...
cum_error = sapply(error, function(x){x^2 %>% cumsum()}) %>%
  ...
  setNames( c('AR', 'AR_PC', 'Ridge', 'LASSO', 'date' ) )
```
