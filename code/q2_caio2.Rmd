...
```{r}
# Import the data
raw_data = read_csv('C:/Users/Caio Garzeri/OneDrive - puc-rio.br/Econometria IV/2021-12.csv')
...
```{r}
MSE <- data.frame (lag1 = numeric((nrow(dataYlag4) - rolling_window)),
...
for(a in 1:(nrow(dataYlag4) - rolling_window)){
  ...
  MSE_RIDGE = (Yobs1 - pred)^2
  MSE$lag1[a] <- MSE_RIDGE
  ...
  MSE_RIDGE = (Yobs2 - pred)^2
  MSE$lag2[a] <- MSE_RIDGE
  ...
  MSE_RIDGE = (Yobs3 - pred)^2
  MSE$lag3[a] <- MSE_RIDGE
  ...
  MSE_RIDGE = (Yobs4 - pred)^2
  MSE$lag4[a] <- MSE_RIDGE
}
...
start_date <- as.Date("1999-03-01")
end_date <- as.Date("2021-11-30")
month_sequence <- seq(start_date, end_date, by = "months")
date <- tail(month_sequence, 257)
